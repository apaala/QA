    ...
    for index, row in technique.iterrows():
        ...
        if data_type == 'raw' and tname in raw_4_file_format_techniques:
            ...
            if check_raw_files['Req'].all() == True and check_raw_files['Opt'].all() == True:
                ...
                logger.error(f"All Required Files for {tname} and Aliquot {aliquot} are NOT present for following lanes %s ",",".join(str(check_raw_files[check_raw_files.eq(False).any(1)]["Lane"])))
